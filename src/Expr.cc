...
bool
PURRS::Expr::has_x_function(bool any_x_function, const Expr& x) const {
  const Expr& e = *this;
  if (e.is_a_add() || e.is_a_mul()) {
    for (unsigned i = e.nops(); i-- > 0; )
      if (e.op(i).has_x_function(any_x_function, x))
	return true;
    return false;
  }
  else if (e.is_a_power()) {
    if (e.arg(0).has_x_function(any_x_function, x)
	|| e.arg(1).has_x_function(any_x_function, x))
      return true;
    return false;
  }
  else if (e.is_a_function()) {
